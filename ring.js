/* The g−2 storage ring, seen from above — the figure that opens stage 03.
   Muons circulate in a 14 m ring; each carries a spin arrow that precesses
   ahead of its momentum (the anomalous precession the experiment measures).
   Muons decay; the positron curls inward and hits one of the 24 calorimeter
   stations on the inside of the ring. The pointer nudges nearby muons.
   Static frame under prefers-reduced-motion; the loop pauses while the
   figure is off screen or the tab is hidden. No dependencies. */
(function () {
    "use strict";
    var canvas = document.getElementById("ring");
    if (!canvas || !canvas.getContext) return;
    var ctx = canvas.getContext("2d", { alpha: false });
    var figure = canvas.parentElement;
    var still = document.getElementById("ring-still");
    var live = document.getElementById("ring-live");

    var GROUND = "#131311"; // --ink-1
    var STEEL = "138, 133, 116"; // --paper-3, for the magnet
    var IVORY = "226, 222, 212"; // --paper, muons
    var AMBER = "224, 166, 60"; // --amber: spin arrows, positrons, calorimeters
    var MOSS = "168, 179, 62"; // --moss, the ideal orbit
    ...
    function resize() {
        ...
        narrow = W < 700;
        // The panel exists for the ring, so centre it and leave room for the
        // spin arrows, which reach outside the magnet.
        cx = W * 0.5;
        cy = H * 0.5;
        R = Math.min(W, H) * 0.44;
    ...
    function update() {
        ...
        // The caption promises pointer interaction, which a still cannot keep.
        if (still && live) {
            still.hidden = !reduceMQ.matches;
            live.hidden = reduceMQ.matches;
        }
    }
    ...
    figure.addEventListener("pointerleave", function () { pointer.t = 0; });

    if ("IntersectionObserver" in window) {
        new IntersectionObserver(function (entries) {
            visible = entries[0].isIntersecting;
            update();
        }, { threshold: 0.02 }).observe(figure);
    }
    ...
    // Web fonts change the panel's height, and with it the ring's radius.
